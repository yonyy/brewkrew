...
class BreweryCard extends React.PureComponent {
	...
	render() {
		...
		const reviewCount = hasYelp
			? brewery.yelp.businesses[0].review_count
			: null;
		const href = hasYelp
			? brewery.yelp.businesses[0].url
			: 'javascript:void(0)';
			...
			<div className="bk-card-container">
				<a target="_blank" href={href} className="bk-card">
					<div className="bk-card-info">
						...
						<p>{brewery.address}</p>
						{brewery.distance && (
							<p>{precisionRound(brewery.distance, 2)} miles</p>
						)}
						<p>
							{rating && reviewCount ? (
								<React.Fragment>
									{rating}{' '}
									<i aria-label="Star" className="fas fa-star bk-icon" />{' '}
									{reviewCount} Reviews
								</React.Fragment>
							) : (
					...
					<div className="bk-card-status">
						{brewery.visited ? (
							<i
								aria-label="Brewery has been visited"
								className="fas fa-check bk-icon"
							/>
						) : null}
					</div>
				</a>
